        ...
        for zrange in ('highz', 'lowz', 'midz'):
            ...
            for l, cfile in enumerate(files):
                ...
                for profile in ('lorentzian','gaussian'):
                    for minusdnu in (10,20,40,50,60,80):

                        TIM_params['profile'] = profile
                        TIM_params['diff_btw_freq_resol_and_fwhm'] = minusdnu 

                        TIM_params['run_name'] = f"pySIDES_from_uchuu_{profile}_TIM_tile{l}_{zrange}_{tile_sizeRA}deg_{tile_sizeDEC}deg_res{TIM_params['pixel_size']:.0f}arcsec_dnu{TIM_params['freq_resol']/1e9:.1f}GHz_minus{TIM_params['diff_btw_freq_resol_and_fwhm']:.1f}percents_forfwhm"
                        file = TIM_params['output_path'] +  TIM_params['run_name'] + '_full_de_Looze_smoothed_MJy_sr.fits' 
                        if(not os.path.isfile(file) ): make_cube(cat, params_sides, TIM_params)